...
internal abstract class ReleasePublishTask @Inject constructor(
    ...
    fun publish() {
        ...
        try {
            ...
            //TODO 删除这一块代码
            val binariesDir = targetBuildDir.get().resolve("compose/binaries")
            if (binariesDir.exists()) {
                logger.lifecycle("=== 产物目录结构 ($binariesDir) ===")
                binariesDir.walkTopDown().forEach { file ->
                    val relative = binariesDir.toPath().relativize(file.toPath())
                    val type = if (file.isDirectory) "DIR" else "FILE"
                    logger.lifecycle("  $type  $relative")
                }
            } else {
                logger.lifecycle("=== compose/binaries 目录不存在 ===")
            }
